...
- **グリッド変換**: メッシュコード付きCSVまたはセルIDリストを、重心割当・
  ...
  面積按分は推計値フラグ付き、整数化（四捨五入／最大剰余法）対応。
  出力はCSV / GeoJSON / Parquet / **GeoParquet 1.1**（WKBジオメトリ、
  CRS: OGC:CRS84）。Parquet生成はブラウザ内で完結します
  （parquet-wasm を初回出力時に遅延読み込み）。
...
- 出力は QGIS 標準の仕組みで GeoPackage / CSV 等へ保存できます。
  GDALのParquetドライバーを含むQGIS（3.28以降の多くのビルド）では、
  出力先の拡張子を `.parquet` にするだけでGeoParquet保存も可能です。
